...
<template>
  <article
    class="group relative flex h-full min-h-[22.5rem] flex-col overflow-hidden rounded-xl border border-slate-200 bg-white shadow-sm transition hover:border-slate-300 hover:shadow-md"
  >
    ...
    >
      <div class="flex h-48 w-full shrink-0 items-center justify-center bg-white px-6 pt-6">
        <img
          v-if="!imageFailed"
          :src="product.image"
          :alt="imageAlt"
        ...
        <div
          v-else
          class="flex h-36 w-36 items-center justify-center rounded-lg bg-slate-50 text-center text-xs text-slate-400"
          role="img"
          :aria-label="imageAlt"
        >
          Imagem indisponível
      ...
      <div class="flex flex-1 flex-col gap-2 px-4 pb-4 pt-3">
        <h3 class="line-clamp-2 h-10 text-sm font-semibold leading-5 text-slate-900">
          {{ product.title }}
        </h3>

        <p class="h-6 text-base font-bold leading-6 text-violet-700">
          {{ formatPrice(product.price) }}
        </p>

        <p class="h-4 truncate text-xs capitalize leading-4 text-slate-500">
          {{ product.category }}
        </p>
